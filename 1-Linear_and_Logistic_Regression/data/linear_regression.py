...
def compute_objective(X: np.ndarray, y: np.ndarray, w: np.ndarray) -> float:
    ...
    y_pred_train = np.matmul(X, w.reshape(-1,1))
    squared_error = (y_pred_train - y.reshape(-1,1))**2
    objective = 0.5 * np.sum(squared_error)
    ###################################################################
    #                        END OF YOUR CODE                         #
    ###################################################################
...
def batch_gradient_descent(
    ...
    *,
    ...
    for current_epoch_number in range(max_epochs):
        ...
        ###################################################################
        # raise NotImplementedError("TODO: Add your implementation here.")
        gradient = np.matmul(X_train.T, np.matmul(X_train, w.reshape(-1, 1))) - np.matmul(X_train.T, y_train.reshape(-1, 1))
...
def stochastic_gradient_descent(
    ...
    for current_epoch_number in range(max_epochs):
        for x_data_point, y_data_point in zip(X_train, y_train):
            ...
            ###################################################################
            # raise NotImplementedError("TODO: Add your implementation here.")
            x1 = x_data_point.reshape(1,-1)
            gradient = np.matmul(x1.T, np.matmul(x1, w.reshape(-1, 1))) - x1.T * y_data_point
            gradient = gradient.squeeze()
            w -= eta * gradient
            assert np.any(w != 0.0), "You are asked to update w properly"
            ###################################################################
...
def closed_form(
    ...
    *,
    ...
    ###################################################################
    #raise NotImplementedError("TODO: Add your implementation here.")
    w = np.linalg.inv(np.matmul(X_train.T,X_train) + reg * np.diag(np.ones(X_train.shape[1])))
    w = np.matmul(np.matmul(w, X_train.T),y_train.reshape(-1,1))
    w = w.squeeze()
    ###################################################################
    #                        END OF YOUR CODE                         #
    ###################################################################
...
def closed_form_locally_weighted(
    ...
    #raise NotImplementedError("TODO: Add your implementation here.")
    r = np.diag(r_train)
    w = np.linalg.inv(np.matmul(np.matmul(X_train.T,r),X_train))
    w = np.matmul(np.matmul(np.matmul(w,X_train.T),r),y_train)
    w = w.squeeze()
    # R = np.diag(r_train)
    # w = (np.linalg.inv(X_train.T @ R @ X_train) @ X_train.T @ R @ y_train).squeeze()
    ###################################################################
    #                        END OF YOUR CODE                         #
    ###################################################################
    assert w is not None, "You are asked to update w properly"
...
def compute_y_space(
    ...
    for idx, x_point in enumerate(x_space):
        ...
        #########################################################################
        # raise NotImplementedError("TODO: Add your implementation here.")
        r_train = np.exp(-(x_point - x_train)**2 / (2 * tau**2))
        w = closed_form_locally_weighted(X_train,y_train,r_train)
        w = w.reshape(-1,1)
        x_space = np.array([1,x_point])
        y_space[idx] = np.matmul(x_space,w)
