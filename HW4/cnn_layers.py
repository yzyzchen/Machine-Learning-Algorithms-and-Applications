...
def conv_forward(x, w):
    ...
    for n in range(N):  # iterate over the batch
        for f in range(F):  # iterate over the number of filters
            for j in range(H_prime):
                for i in range(W_prime):
                    # Select the current region of interest
                    region = x[n, :, j:j+HH, i:i+WW]
                    # Perform elementwise multiplication and sum
                    out[n, f, j, i] = np.sum(region * w[f, :, :, :])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w)
...
def conv_backward(dout, cache):
    ...
    for n in range(N):  # For each image in the batch
        for f in range(F):  # For each filter
            for j in range(H_prime):
                for i in range(W_prime):
                    # Gradient of output (dout) with respect to input volume (dx)
                    dx[n, :, j:j+HH, i:i+WW] += w[f] * dout[n, f, j, i]
                    # Gradient of output (dout) with respect to weights (dw)
