...
class Trainer:
    ...
    def run(self, type):
        ...
        for _ in range(self.max_iters):
            ...
            output, loss = self.model(x, y)  # Forward pass: Compute predicted y and loss if targets are provided
            if loss is not None:
                loss.backward()  # Backward pass
                torch.nn.utils.clip_grad_norm_(self.model.parameters(), 1.0)  # Clip gradients to avoid exploding gradients
                self.optimizer.step()
            ###########################################################################
            #                             END OF YOUR CODE                            #
            ###########################################################################
